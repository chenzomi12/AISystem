...
本文将通过介绍 AI 编译器的设计目标、定义、发展阶段等角度，详细探讨 AI 编译器的历史发展。
...
如下图所示，AI 编译器的发展应该分为三个阶段：朴素 AI 编译器（阶段一）、专用 AI 编译器（阶段二）、通用 AI 编译器（阶段三），接下来我们将会按照这三个阶段的顺序详细介绍 AI 编译器的发展阶段。 
...
阶段一朴素 AI 编译器，基于神经网络的编程模型，主要进行了 graph 图和 ops 算子两层抽象；阶段二专用 AI 编译器，在表达形式和性能优化上取得了较大的进展；阶段三通用 AI 编译器，将会针对专用 AI 编译器目前存在的问题进行优化，尤其是在统一表达、自动化优化、泛化优化能力模块化设计等领域进行更多的创新设计。
