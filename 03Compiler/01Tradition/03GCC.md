...
- GCC 优点：1）支持多种编程语言；2）跨平台支持广泛；3）使用广泛且功能完备；4）基于 C 的架构，只需 C++ 编译器即可编译。

- GCC 缺点：1）代码耦合度高，难以独立集成到专用 IDE 中；2）作为单一静态编译器，难以作为 API 集成到其他工具中；3）后期版本代码质量下降；4）代码量庞大约 1500 万行。
