...
训练芯片用于神经网络的训练过程，通常需要更大的芯片面积和功耗预算，注重提供高吞吐量的计算能力。推理芯片则用于训练后模型的部署和执行，更加强调低功耗和实时性。典型的训练卡有英伟达 A100、英伟达 DGX 系列、AMD Instinct MI 系列、谷歌 TPU 系列、Intel Nervana NNP 系列、Huawei Ascend 910 等。
