...
======= 图片记得按顺序，按规范命名，我给你挪了位置，大致命名好了，剩下靠你按顺序调整一下。
...
========= 深入解释为什么需要异构？异构的原因，异构的好处？场景是什么？不能比视频还简单，要深入深入哈。
...
====== 为什么呢？加一个具体的场景，例如玩游戏、画面渲染，技术一点，深入一下哈。
...
======== 用 GPT4 或者 kimi 扩写一下，上面感觉有点衔接不是很好。
...
2. **工艺和封装支撑**：工艺封装持续进步，10nm 以下芯片从 2D->3D->4D。Chiplet 封装使得在单芯片层次，可以构建规模数量级提升的超大系统。系统规模越大，超异构的优势越明显；。

========10nm 以下芯片从 2D->3D->4D。？？怎么理解，不懂哈

3. **系统架构持续创新**：通过架构创新，在单芯片层次，实现多个数量级的性能提升。异构编程很难，超异构编程更是难上加难。如何更好地驾驭超异构，是成败的关键。

======== 用 GPT4 或者 kimi 润色，上面感觉有点衔接不是很好，读不懂。
