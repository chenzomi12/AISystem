...
- AI芯片设计旨在实现低成本高效率的AI模型执行，关键设计指标包括：精度 (Accuracy)、吞吐量 (Throughput)、时延 (Latency)、能耗 (Energy)、系统价格 (Cost)、易用性 (Flexibility)。

- 计算单位有：OPS，OPS/W，MACs，FLOPs，MAC。

- 计算性能仿真方面因为存在内存受限与算力受限的影响，因此需要深入研究算术强度和操作字节比，通过Roofline模型更好地根据硬件平台的算力和带宽上限，评估最大性能，平衡带宽和算力。
